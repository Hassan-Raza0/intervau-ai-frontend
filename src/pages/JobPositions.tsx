...
import { Briefcase, Plus, Search, Filter, ChevronDown, MapPin, Users, Clock } from 'lucide-react';

interface JobPositionsProps {
  onNavigate: (page: string) => void;
}

export default function JobPositions({ onNavigate }: JobPositionsProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Job Positions</h1>
        ...
        <div className="space-y-4">
          {filteredJobs.map((job) => (
            <div key={job.id} className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-all group">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right flex-shrink-0 ml-4">
                  <div className={`inline-block px-4 py-2 rounded-full text-sm font-semibold mb-3 ${getStatusColor(job.status)}`}>
              ...
              <div className="grid md:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-green-600">{job.qualified}</p>
                  <p className="text-xs text-gray-500 mt-1">Qualified</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600">{job.department}</p>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Create New Position</h2>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Job Title</label>
                  <input type="text" placeholder="e.g., Senior Developer" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g., San Francisco, CA" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
              </div>
