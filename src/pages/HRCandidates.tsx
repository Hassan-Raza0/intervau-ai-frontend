...
interface HRCandidatesProps {
  onNavigate: (page: string) => void;
}

export default function HRCandidates({ onNavigate }: HRCandidatesProps) {
  ...
  const stats = [
    { label: 'Total Candidates', value: candidates.length, color: 'from-blue-500 to-cyan-500' },
    { label: 'Qualified', value: candidates.filter(c => c.status === 'qualified').length, color: 'from-green-500 to-emerald-500' },
    { label: 'In Interview', value: candidates.filter(c => c.status === 'interview').length, color: 'from-purple-500 to-pink-500' },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Browse and review all candidates in your pipeline</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-4 px-4 text-sm font-semibold text-gray-700">Candidate</th>
                  ...
                  <th className="text-center py-4 px-4 text-sm font-semibold text-gray-700">Applied</th>
                  <th className="text-center py-4 px-4 text-sm font-semibold text-gray-700">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredCandidates.map((candidate) => (
                  <tr key={candidate.id} className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        <div className="bg-gradient-to-br from-blue-500 to-cyan-500 w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0">
                          <span className="text-white font-semibold text-sm">{candidate.name.charAt(0)}</span>
                        </div>
                        <div>
                    ...
                    <td className="py-4 px-4 text-gray-700 text-sm">{candidate.position}</td>
                    ...
                    <td className="py-4 px-4 text-center text-gray-700 font-medium">{candidate.interviews}</td>
                    <td className="py-4 px-4 text-center text-sm text-gray-600">{candidate.appliedDate}</td>
                    <td className="py-4 px-4 text-center">
                      <button
